...
export const getCategories = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: USE_SNACK_BAR,
      payload: {
        message: 'Простите, категории опаздывают, но не переживайте мы уже выслали им в помощь вертолёт',
        success: true,
      },
    });
  }
...
export const deleteCategory = categoryID => async (dispatch) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      dispatch({
        type: USE_SNACK_BAR,
        payload: { message: 'Мне давно она не нравилась. Спасибо что удалил', success: true },
      });
    ...
    dispatch({
      type: USE_SNACK_BAR,
      payload: {
        message: 'Оу простите, но категория слишком стойкая,не получается её удалить, повторите позже',
        success: true,
      },
    });
  }

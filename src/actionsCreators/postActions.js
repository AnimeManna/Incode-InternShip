...
export const getPostForUpdate = (postId) => {
    return async dispatch => {
        ...
        if (response.success) {
            dispatch({type: GET_POST_UPDATE_SUCCESS, payload: {response, isLoading: false, isLoaded: true}});
        } else {
            dispatch({type: GET_POST_UPDATE_ERROR, payload: {isLoading: false, isLoaded: true}})
        }
    }
}
...
export const updatePost = (data, postID) => {
    return async dispatch => {
        dispatch({type: UPDATE_POST_START, payload: {isLoading: true, isLoaded: false}});
        try {

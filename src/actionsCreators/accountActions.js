...
export const getUserPostsById = userId => async (dispatch) => {
  dispatch({ type: GET_USER_BY_ID_START, payload: { isLoaded: false, isLoading: true } });
  const user = await axiosProviders.getRequestWithToken(`post/author/${userId}`);
  if (user.success) {
    dispatch({
...
export const dispatchUserID = userId => (dispatch) => {
